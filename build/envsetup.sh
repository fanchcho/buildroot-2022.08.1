...
function choose_board()
{
	...
	while true; do
		...
		if [ "$INDEX" -eq -1 ]; then
			echo "Lunching for non-imx6ull boards..."
			unset TARGET_OUTPUT_DIR
			unset IMX_BUILD_CONFIG
			break;
		fi

		if echo $INDEX | grep -vq [^0-9]; then
			IMX_BUILD_CONFIG="${RK_DEFCONFIG_ARRAY[$INDEX]}"
			[ -n "$IMX_BUILD_CONFIG" ] && break
		fi
...
function launch_imx6ull()
{
	...
	echo "==========================================="
	echo
	echo "#TARGET_BOARD=`echo $IMX_BUILD_CONFIG | cut -d '_' -f 2`"
	echo "#OUTPUT_DIR=output/$TARGET_DIR_NAME"
	echo "#CONFIG=${IMX_BUILD_CONFIG}_defconfig"
	echo
	echo "==========================================="

	make -C ${BUILDROOT_DIR} O="$TARGET_OUTPUT_DIR" \
		"$IMX_BUILD_CONFIG"_defconfig
...
function main()
{
	...
	case $RK_DEFCONFIG_ARRAY_LEN in
		0)
			...
			;;
		1)
			IMX_BUILD_CONFIG=${RK_DEFCONFIG_ARRAY[0]}
			;;
		*)
